...
import cssComponents from "@polkadot/react-components/styles/components";
import cssForm from "@polkadot/react-components/styles/form";
import cssMedia from "@polkadot/react-components/styles/media";
import cssRx from "@polkadot/react-components/styles/rx";
import cssSemantic from "@polkadot/react-components/styles/semantic";
import cssTheme from "@polkadot/react-components/styles/theme";
...
function countBrightness(uiHighlight: string | undefined): number {
	...
	return PARTS.reduce(
		(b, p, index) => b + parseInt(hc.substr(p, 2), 16) * FACTORS[index],
		0
	);
}
...
export default createGlobalStyle<Props & ThemeProps>(
  ...
  .ui--MenuItem {
    ...
    &:hover:not(.isActive) .ui--Badge {
      background: ${
				countBrightness(uiHighlight) < BRIGHTNESS
