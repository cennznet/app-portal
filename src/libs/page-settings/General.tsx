...
import type { Option } from "../apps-config/settings/types";
...
function General({ className = "" }: Props): React.ReactElement<Props> {
	...
	const ledgerConnOptions = useMemo(
		() =>
			_ledgerConnOptions.filter(
				({ value }) => !isElectron || value !== "webusb"
			),
		[isElectron]
	);
	...
	useEffect((): void => {
		...
		const needsReload =
			prev.apiUrl !== state.apiUrl || prev.prefix !== state.prefix;
	...
	const { i18nLang, icon, ledgerConn, uiTheme } = state;
		...
		<div className={className}>
			...
			{hasLedgerChain && (
				<>
					<div className="ui--row">
						<Dropdown
							defaultValue={hasWebUsb ? ledgerConn : ledgerConnOptions[0].value}
