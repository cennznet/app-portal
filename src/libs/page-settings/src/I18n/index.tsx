...
import StringInput from "@polkadot/app-settings/I18n/StringInput";
...
async function retrieveEnglish(): Promise<StringsMod> {
	...
	const strings: Strings[] = await Promise.all(
		paths.map((path) => retrieveJson(`en/${path}`) as Promise<Strings>)
	);
...
async function retrieveAll(): Promise<Defaults> {
	...
	const translations = missing.length
		? await Promise.all(
				missing.map((lng) => retrieveJson(`${lng}/translation.json`))
		  )
		: [];
...
function doDownload(strings: Strings, withEmpty: boolean): void {
	...
	FileSaver.saveAs(
		new Blob([JSON.stringify(sanitized, null, 2)], {
...
function progressDisplay(
	[done, total, _]: [number, number, number] = [0, 0, 0]
): { done: number; progress: string; total: number } {
	return {
		done,
...
function Translate({ className }: Props): React.ReactElement<Props> {
	...
	const [{ english, keys, modules }, setDefaults] = useState<Defaults>({
		english: {},
		keys: [],
		modules: [],
	});
	const [lng, setLng] = useState<string>("zh");
	const [[modProgress, allProgress], setProgress] = useState<Progress>([
		[0, 0, 0],
		{},
	]);
	...
	const _setString = useCallback(
		(key: string, value: string): void => {
			setStrings((strings: Strings | null): Strings | null =>
				strings ? { ...strings, [key]: value } : null
			);
	...
	return (
		<main className={className}>
					...
					<Columar.Column>
						<div>
							<Dropdown
								...
								value={lng}
							/>
							{t<string>("{{done}}/{{total}}, {{progress}}% done", {
			...
			<Button.Group>
				...
				<Button
					icon="download"
					label={t<string>("Generate {{lng}}/translation.json", {
						replace: { lng },
					})}
					onClick={_onDownload}
				/>
