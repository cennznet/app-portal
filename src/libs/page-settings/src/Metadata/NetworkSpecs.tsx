...
function NetworkSpecs({
	...
	const [qrData, setQrData] =
		useState<NetworkSpecsStructWithType>(initialState);
	const debouncedQrData = useDebounce(qrData, 500);

	const reducer = (
		state: NetworkSpecsStructWithType,
		delta: Partial<NetworkSpecsStructWithType>
	): NetworkSpecsStructWithType => {
		const newState = {
			...state,
			...delta,
		};
				...
				<td>
					<div className="settings--networkSpecs-name">
						<Input
							className="full"
							help={t<string>(
								"Name of the network. It is only for display purposes."
							)}
							isDisabled
			...
			<tr>
				<td>
					<Input
						className="full"
						help={t<string>(
							"Decimals decides the smallest unit of the token, which is 1/10^decimals"
						)}
						isDisabled
			...
			<tr>
				<td>
					<Input
						className="full"
						help={t<string>(
							"Chain type (ethereum compatible or regular substrate)"
						)}
						isDisabled
