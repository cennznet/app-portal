...
function Developer({
	...
	useEffect((): void => {
		const types =
			decodeUrlTypes() || (store.get("types") as Record<string, unknown>) || {};
	...
	const _saveDeveloper = useCallback((): void => {
		...
		try {
			...
			onStatusChange({
				action: t(
					`Error saving your custom types. ${(error as Error).message}`
				),
				status: "error",
			});
		}
			...
			<div className="ui--row">
				<div className="full">
					<Trans i18nKey="devConfig">
						<div className="help">
							If you are a development team with at least a test network
							available, consider adding the types directly{" "}
							<a
							...
							, allowing out of the box operation for your spec &amp; chains,
							both for you and anybody trying to connect to it. This is not a
							replacement for your chain-specific UI, however doing so does help
							in allowing users to easily discover and use with zero-config.
						</div>
					</Trans>
				</div>
			</div>
