...
function WarmUp(): React.ReactElement {
	...
	const registrars = useCall<unknown>(
		isApiReady && api.query.identity?.registrars
	);
	const issuance = useCall<unknown>(
		isApiReady && api.query.balances?.totalIssuance
	);
	const historyDepth = useCall<unknown>(
		isApiReady && api.query.staking?.historyDepth
	);
