...
function checkVisible(
	...
	return (
		findMissingApis(api, needsApi, needsApiInstances, needsApiCheck).length ===
		0
	);
}
...
function Menu({ className = "" }: Props): React.ReactElement<Props> {
	...
	const sudoKey = useCall<AccountId>(
		apiProps.isApiReady && apiProps.api.query.sudo?.key
	);
	...
	return (
		<div
			className={`${className}${
				!apiProps.isApiReady || !apiProps.isApiConnected ? " isLoading" : ""
			} highlight--bg`}
		>
					...
					<ul className="menuItems">
						{visibleGroups.map(
							({ name, routes }): React.ReactNode => (
								<Grouping
									isActive={
										activeRoute && activeRoute.group === name.toLowerCase()
									}
									key={name}
									name={name}
									routes={routes}
