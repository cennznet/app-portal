...
import { createWsEndpoints } from "./index";
...
describe("WS urls are all valid", (): void => {
	allEndpoints
		.filter(
			({ value }) => value && isString(value) && !value.includes("127.0.0.1")
		)
		.map(
			({ text, textBy, value }): Endpoint => ({
				name: text as string,
				provider: textBy,
				value,
			})
			...
			it(`${name}:: ${provider}`, (): void => {
				assert(
					value.startsWith("wss://") ||
						value.startsWith("light://substrate-connect/"),
					`${name}:: ${provider} -> ${value} should start with wss:// or light://`
				);
			})
		);
});
			...
			it(`${lastHeader}:: ${text as string}:: ${textBy}`, (): void => {
				assert(
					filtered[index - 1].isHeader ||
						...
						(text as string).localeCompare(
							filtered[index - 1].text as string
						) === 1,
					`${lastHeader}:: ${text as string} needs to be before ${
						filtered[index - 1].text as string
					}`
				);
			});
...
describe("urls are not duplicated", (): void => {
	...
	const filtered = allEndpoints.filter(
		({ isDisabled, isHeader, isUnreachable, text }): boolean => {
			hasDevelopment = hasDevelopment || (!!isHeader && text === "Development");
	...
	it("has no duplicates, e.g. parachain & live", (): void => {
		expect(
			Object.entries(map).filter(([, paths]) => paths.length !== 1)
		).toEqual([]);
	});
});
	...
	allEndpoints
		...
		.forEach(({ name, provider }) =>
			it(`${name}:: ${provider}`, (): void => {
				...
				assert(
					!emoji.test(provider),
					`${name}:: ${provider} should not contain any emojis`
				);
				assert(
					!provider.includes(" ") || provider.toLocaleUpperCase() !== provider,
					`${name}:: ${provider} should not be all uppercase`
				);
			})
		);
});
